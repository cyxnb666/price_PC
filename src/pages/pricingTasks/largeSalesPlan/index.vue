<template>
  <div>
        ...
        <t-row :gutter="[16, 24]">
          ...
          <t-col :span="3">
            <t-form-item label="任务状态" name="planStatus">
              <t-select clearable v-model="formData.planStatus" class="form-item-content" :options="taskStatusOptions"
                placeholder="请选择任务状态" />
            </t-form-item>
          </t-col>
        </t-row>
        ...
        <t-table :columns="columns" :data="data" :rowKey="rowKey" :verticalAlign="verticalAlign" :hover="hover"
          ...
          <template #op="slotProps">
            ...
            <t-button theme="primary" variant="text" class="t-button-link" @click="handleTransfer(slotProps)"
              v-if="slotProps.row.collectorId && slotProps.row.status == '2'" :loading="transferLoading"
            ...
            <t-button theme="primary" variant="text" class="t-button-link" @click="handleAssign(slotProps)"
              v-else-if="slotProps.row.status == '1'" :loading="assignLoading"
    ...
    <t-dialog :header="dialogTitle" :visible.sync="dialogVisible" :destroyOnClose="true" :onClose="onDialogClose"
      width="700px" class="custom-dialog">
...
export default Vue.extend({
  ...
  mounted() {
    if (sessionStorage.getItem(this.$route.name)) {
      ...
      if (this.paramsData.condition) {
        this.formData.areaCode = this.paramsData.condition.areaCode || '';
        this.formData.stallType = this.paramsData.condition.stallType || '';
        this.formData.stallVest = this.paramsData.condition.stallVest || '';
        this.formData.stallId = this.paramsData.condition.stallId?.length ? this.paramsData.condition.stallId[0] : '';
        this.formData.varietyId = this.paramsData.condition.varietyId || '';
        this.formData.collectorId = this.paramsData.condition.collectorId || '';
        this.formData.planStatus = this.paramsData.condition.planStatus || '';
      }

      if (this.paramsData.pageNo) {
        this.pagination.pageNo = this.paramsData.pageNo;
      }

      if (this.paramsData.pageSize) {
        this.pagination.pageSize = this.paramsData.pageSize;
      }
    }
    this.getList();
    this.getAreaList();
  ...
  methods: {
    // 转派专用的采价员获取方法
    getTransferPersonnelOptions(stallId) {
      const params = {
        condition: {
          primaryKey: stallId,
        },
      ...
      return this.$request
        .post('/web/collector/selectStallChooseCollectors', params)
        .then((res) => {
          if (res.retCode === 200) {
    ...
    handleAssign({ row }) {
      ...
      this.getTransferPersonnelOptions(row.stallId)
        .then(() => {
