  ...
  <div>
    <t-card class="list-card-container" :bordered="false">
            ...
            <t-row :gutter="[16, 24]">
              <t-col :flex="1">
                <t-form-item label="姓名" name="userName">
                  <t-input v-model="formData.userName" class="form-item-content" type="search" placeholder="请输入姓名"
                    :style="{ minWidth: '134px' }" />
                </t-form-item>
              </t-col>
              <t-col :flex="1">
                <t-form-item label="手机号" name="userMobile">
                  <t-input v-model="formData.userMobile" class="form-item-content" placeholder="请输入手机号"
                    :style="{ minWidth: '134px' }" />
                </t-form-item>
              </t-col>
              <t-col :flex="1">
                <t-form-item label="用户类型" name="userType">
                  <t-select clearable v-model="formData.userType" class="form-item-content`" :options="SEARCH_USER_TYPE"
                    placeholder="请选择用户类型" />
                </t-form-item>
              </t-col>
            </t-row>
      ...
      <div class="table-container">
        <t-table :columns="columns" :data="data" :rowKey="rowKey" :verticalAlign="verticalAlign"
          height="calc(100vh - 300px)" :hover="hover" :pagination="pagination" :loading="dataLoading"
          ...
          <template #enabled="{ row }">
            <t-switch size="large" v-model="row.enabled" :customValue="['1', '0']"
    ...
    <t-dialog :header="title" :destroyOnClose="true" :onClose="onCancel" :cancelBtn="null" :confirmBtn="null"
      :visible.sync="confirmVisible" v-loading="loading">
      <t-form ref="form" @submit="onConfirmDelete" :data="form" :rules="FORM_RULES" label-align="left"
        :label-width="90">
        <t-form-item label="姓名" name="userName">
        ...
        <t-form-item label="用户类型" name="userType">
          <div v-for="(check, i) in filteredUserTypes" :key="i + 'check'" style="margin-right: 10px">
            <t-checkbox :key="check.value" :checked="form.userType == check.value" :label="check.label" :disabled="true"
              @change="(checked) => checkChange(checked, check.value)"></t-checkbox>
          </div>
        </t-form-item>
        <t-form-item label="用户角色" name="roleId">
          <t-select clearable v-model="form.roleId" class="form-item-content`" :options="roleList"
            placeholder="请选择用户类型" />
        </t-form-item>
        <t-form-item label="行政区划" name="areaCodes">
          <t-tree-select multiple :min-collapsed-num="minCollapsedNum" clearable v-model="form.areaCodes"
            :treeProps="treeProps" :data="areaList" placeholder="请选择" />
        </t-form-item>
  ...
  methods: {
    ...
    onConfirmDelete({ validateResult }) {
      ...
      if (validateResult === true) {
        const filteredAreaCodes = this.form.areaCodes.filter(code => code !== 'all');
        this.loading = true;
        let params = {
          condition: {
            ...this.form,
            areaCodes: filteredAreaCodes,
